...
for CHR in ${CHR_LIST[@]}; do
  if [ $CHR != $CHR_MIN ]; then
    ...
    CHR_BASENAME="$TMP_DIR/knockoffs/knockoffs_chr"$CHR"_res"$RESOLUTION
    echo $CHR_BASENAME >> $MERGE_LIST
  fi
done

# Merge the augmented data from all chromosomes
CHR_BASENAME_FIRST="$TMP_DIR/knockoffs/knockoffs_chr"$CHR_MIN"_res"$RESOLUTION
plink \
...
for CHR in ${CHR_LIST[@]}; do
  # Basename for the augmented genotype file for this chromosome
  GRP_FILE="$TMP_DIR/knockoffs/knockoffs_chr"$CHR"_res"$RESOLUTION"_grp.txt"
